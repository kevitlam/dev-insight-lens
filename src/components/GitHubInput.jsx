  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <input
            ...
            disabled={disabled}
            className={`w-full px-4 py-3 border-2 rounded-xl text-base focus:outline-none focus:ring-0 transition-all duration-200 ${
              !isValid 
      ...
      <div className="mt-4 text-center">
        <span className="text-xs text-gray-500 mr-3">Try examples:</span>
        {['octocat', 'defunkt', 'mojombo'].map((username) => (
          <button
            ...
            disabled={disabled}
            className="text-xs text-gray-500 hover:text-gray-900 underline mr-3 disabled:text-gray-300 transition-colors duration-200"
          >
            {username}
          </button>
        ))}
